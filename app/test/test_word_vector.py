...
PATH_RESOURSE = "test_resource/word_vector/"
...
def model(setup_file):
    _ = WordVector(PATH_FILE, PATH_RESOURSE)
    yield _


def test_instance_able(model):
    assert isinstance(model, WordVector)


def test_create_models_word_vector(model):
    model.generate()
    assert os.path.isfile("./" + PATH_RESOURSE + "あばばばば.word_vector")
    assert os.path.isfile("./" + PATH_RESOURSE + "トロッコ.word_vector")
